declare const symIsDefaultFun: unique symbol;
interface TypeArithmetics<T> {
    conv: {
        (a: any): T;
...
interface PrefixOptions<T> {
    autoPrefix?: boolean;
    prefixMin?: T;
    prefixMax?: T;
    formatPrefixDefault?: 'all' | 'none';
}
interface SimplifyOptions<T> extends PrefixOptions<T> {
    system?: string;
}
type Options<T> = FormatOptions<T> & SimplifyOptions<T> & {
    type?: Partial<TypeArithmetics<T>>;
    definitions?: Partial<NullableDefinitions> & {
        skipBuiltIns?: boolean;
    };
};
interface PrefixGroups {
...
interface UnitPropsWithQuantity extends UnitPropsCommons {
    quantity: string;
    value: number;
}
interface UnitPropsStringValue extends UnitPropsCommons {
    quantity?: undefined;
    value: string;
}
interface UnitPropsTupleValue extends UnitPropsCommons {
    quantity?: undefined;
    value: [number, string];
}
type UnitProps = UnitPropsStringValue | UnitPropsWithQuantity | UnitPropsTupleValue;
interface UnitPropsExtended<T> {
    name: string;
    quantity?: string;
    value: T;
...
interface NullableDefinitions extends Omit<Definitions, 'units'> {
    units: Record<string, UnitProps | null | false | undefined>;
}
interface DefinitionsExtended<T> {
    prefixGroups: PrefixGroups;
